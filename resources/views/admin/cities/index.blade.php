        ...
        <div class="overflow-x-auto">
            <table class="table-hover">
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <form method="POST" action="{{ route('admin.cities.toggle-status', $city) }}" class="inline">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="badge {{ $city->enable ? 'bg-success' : 'bg-danger' }}">
                                        {{ $city->enable ? 'Active' : 'Inactive' }}
                                    </button>
                                </form>
                            </td>
                            <td class="text-center">
                                <div class="flex items-center justify-center gap-3">
                                    <a href="{{ route('admin.cities.edit', $city) }}" class="hover:text-primary">
                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                                            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                                            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                                        </svg>
                                    </a>
                                    <form method="POST" action="{{ route('admin.cities.destroy', $city) }}" class="inline" onsubmit="return confirm('Are you sure?')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-4 text-gray-500">No cities found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
