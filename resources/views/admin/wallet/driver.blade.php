...
<div class="space-y-5">
    <div class="flex flex-wrap items-center justify-between gap-4">
        <h2 class="text-xl font-bold">Driver Wallet Transactions</h2>
        <a href="{{ route('admin.wallet.driver.export', request()->query()) }}"
    ...
    <div class="panel">
        <form method="GET" action="{{ route('admin.wallet.driver') }}" class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
            <div class="lg:col-span-2">
                <label class="block mb-1 text-xs font-semibold tracking-wide text-gray-500 uppercase">Search</label>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Driver name, mobile, or transaction ID" class="form-input" />
            </div>
            <div>
            ...
            <div class="flex items-end gap-2">
                ...
                @if($hasFilters)
                    <a href="{{ route('admin.wallet.driver') }}" class="btn btn-outline-primary">Clear</a>
                @endif
            </div>
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="table-hover">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-4 text-gray-500">No driver wallet transactions found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
