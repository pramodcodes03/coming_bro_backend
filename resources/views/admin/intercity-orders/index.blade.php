<x-layout.admin>
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Intercity Orders</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Manage all intercity ride orders</p>
            </div>
        </div>
        ...
        <!-- Search & Filters -->
        <div class="panel">
            <form method="GET" action="{{ route('admin.intercity-orders.index') }}" class="flex flex-col gap-4 sm:flex-row sm:items-center">
                ...
                @if(request('search') || request('status'))
                    <a href="{{ route('admin.intercity-orders.index') }}" class="btn btn-outline-primary">Clear</a>
                @endif
            </form>
        </div>
                    ...
                    <tbody>
                        @forelse($orders as $order)
                            <tr>
                                ...
                                <td class="whitespace-nowrap">{{ $order->customer_name ?? '-' }}</td>
                                <td class="whitespace-nowrap">{{ $order->driver_name ?? 'Unassigned' }}</td>
                                <td>
                                    <div class="text-xs">
                                        @if($order->source_city || $order->destination_city)
                                            <span class="font-medium text-gray-900 dark:text-white">{{ $order->source_city ?? '-' }}</span>
                                            <span class="mx-1 text-gray-400">&rarr;</span>
                                            <span class="font-medium text-gray-900 dark:text-white">{{ $order->destination_city ?? '-' }}</span>
                                        @else
                                            -
                                        @endif
                                    </div>
                                ...
                                <td class="whitespace-nowrap">
                                    {{ $order->created_date ? \Carbon\Carbon::parse($order->created_date)->format('d M Y, h:i A') : '-' }}
                                </td>
                                <td class="text-center">
                                    <a href="{{ route('admin.intercity-orders.show', $order->id) }}"
                            ...
                            <tr>
                                <td colspan="9" class="py-8 text-center text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col items-center gap-2">
                                        ...
                                        <span>No intercity orders found.</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
