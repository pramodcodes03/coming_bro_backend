            ...
            <table class="table-hover">
                ...
                <tbody>
                    @forelse($states as $state)
                        <tr>
                            <td>{{ $loop->iteration + ($states->currentPage() - 1) * $states->perPage() }}</td>
                            <td>{{ $state->name }}</td>
                            <td>
                                <form method="POST" action="{{ route('admin.states.toggle-status', $state) }}" class="inline">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="badge {{ $state->enable ? 'bg-success' : 'bg-danger' }}">
                            ...
                            <td class="text-center">
                                <div class="flex items-center justify-center gap-3">
                                    <a href="{{ route('admin.states.edit', $state) }}" class="hover:text-primary">
                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                                            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                                            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                                        </svg>
                                    </a>
                                    <form method="POST" action="{{ route('admin.states.destroy', $state) }}" class="inline" onsubmit="return confirm('Are you sure?')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center py-4 text-gray-500">No states found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
