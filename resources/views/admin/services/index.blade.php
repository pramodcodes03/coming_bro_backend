                ...
                <tbody>
                    @forelse($services as $service)
                        <tr>
                            ...
                            <td>{{ $service->title }}</td>
                            <td>{{ $service->basic_fare_charges }}</td>
                            <td>{{ $service->km_charge }}</td>
                            <td>
                                <form method="POST" action="{{ route('admin.services.toggle-status', $service) }}" class="inline">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="badge {{ $service->enable ? 'bg-success' : 'bg-danger' }}">
                                        {{ $service->enable ? 'Active' : 'Inactive' }}
                                    </button>
                                </form>
                            </td>
                            <td class="text-center">
                                <div class="flex items-center justify-center gap-3">
                                    <a href="{{ route('admin.services.edit', $service) }}" class="hover:text-primary">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-4 text-gray-500">No services found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
