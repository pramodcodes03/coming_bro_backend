...
<div class="space-y-5">
    <div class="flex items-center gap-2">
        <a href="{{ route('admin.vehicle-companies.index') }}" class="hover:text-primary">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                <path d="M19 12H5" /><path d="M12 19l-7-7 7-7" />
            </svg>
        </a>
        <h2 class="text-xl font-bold">{{ isset($company) ? 'Edit' : 'Create' }} Vehicle Company</h2>
    </div>
    ...
    <div class="panel">
        <form method="POST"
              action="{{ isset($company) ? route('admin.vehicle-companies.update', $company) : route('admin.vehicle-companies.store') }}">
            @csrf
            ...
            <div class="mt-5">
                <label class="flex items-center gap-2">
                    ...
                    <input type="checkbox" name="enable" value="1" class="form-checkbox"
                        {{ old('enable', $company->enable ?? false) ? 'checked' : '' }} />
                    <span>Enable</span>
                </label>
            </div>

            <div class="mt-6 flex justify-end gap-3">
                <a href="{{ route('admin.vehicle-companies.index') }}" class="btn btn-outline-danger">Cancel</a>
