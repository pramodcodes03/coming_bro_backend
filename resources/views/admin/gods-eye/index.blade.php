<x-layout.admin>
    {{--
        ...
        re-renders the KPIs, map markers, driver roster and trip details. The map
        uses Google Maps, loaded with the key stored in settings → globalKey.
    --}}
    <div x-data="godsEye({
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Live master control for the entire fleet —
                    track any driver, ride or passenger instantly.</p>
            </div>
            <div class="flex items-center gap-3">
                ...
                <button type="button" @click="togglePolling()"
                    ...
                    :class="!polling ? 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300' : (stale ?
                    ...
                    <span class="relative flex h-2.5 w-2.5">
                        <span x-show="polling && !stale"
                            class="absolute inline-flex h-full w-full animate-ping rounded-full bg-success opacity-75"></span>
                        <span class="relative inline-flex h-2.5 w-2.5 rounded-full"
                            :class="!polling ? 'bg-gray-400' : (stale ? 'bg-warning' : 'bg-success')"></span>
                    </span>
                    <span x-text="!polling ? 'Paused' : (stale ? 'Reconnecting…' : 'Live')"></span>
                </button>
                <div
                    class="hidden items-center gap-2 rounded-lg bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 shadow-sm dark:bg-[#0e1726] dark:text-gray-300 sm:flex">
            ...
            <template x-for="kpi in kpiCards" :key="kpi.key">
                <div class="panel !p-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="min-w-0">
                            <p class="text-xl font-bold leading-tight text-gray-900 dark:text-white" x-text="kpi.value">
                            </p>
                            <p class="truncate text-[11px] font-medium text-gray-500 dark:text-gray-400"
                                :title="kpi.label" x-text="kpi.label"></p>
                        </div>
                    </div>
                </div>
            </template>
            ...
            <div class="panel xl:col-span-2">
                <div class="mb-4 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">Online drivers and in-progress rides,
                    ...
                    <div class="flex items-center gap-3 text-xs">
                        ...
                        <span class="flex items-center gap-1.5"><span class="h-2.5 w-2.5 rounded-full bg-info"></span>On
                            trip</span>
                        <span class="flex items-center gap-1.5"><span
                                class="h-2.5 w-2.5 rounded-full bg-primary"></span>Pickup</span>
                        <span class="flex items-center gap-1.5"><span
                                class="h-2.5 w-2.5 rounded-full bg-danger"></span>Drop</span>
                    </div>
                </div>
                ...
                <div class="relative">
                    <div id="godsEyeMap"
                        class="h-[460px] w-full overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-800 z-0"></div>

                    {{-- Loading / error overlay (shown until the map renders) --}}
                    <div x-show="mapStatus !== 'ready'" x-cloak
                        class="pointer-events-none absolute inset-0 flex flex-col items-center justify-center gap-2 rounded-xl bg-gray-50/90 text-center dark:bg-gray-800/90">
                        ...
                        <p class="text-sm font-semibold text-gray-600 dark:text-gray-300"
                            x-text="mapStatus === 'error' ? 'Map failed to load' : 'Loading map…'"></p>
                        <p class="max-w-xs text-xs text-gray-400" x-show="mapStatus === 'error'">Check the Google Maps
                            key and that the <strong>Maps JavaScript API</strong> is enabled. The roster &amp; trip data
                            below still work.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="panel flex flex-col">
                <div class="mb-4 flex items-center justify-between">
                    ...
                    <span class="rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-bold text-primary">
                        <span x-text="data.kpis.online_drivers"></span> / <span x-text="data.kpis.total_drivers"></span>
                        online
                    </span>
                </div>
                ...
                <div class="mb-3 flex gap-1 rounded-lg bg-gray-100 p-1 dark:bg-gray-800">
                    <template x-for="tab in ['all','online','on trip','offline']" :key="tab">
                        <button type="button" @click="rosterFilter = tab"
                            ...
                            :class="rosterFilter === tab ? 'bg-white text-primary shadow-sm dark:bg-[#0e1726]' :
                ...
                <div class="relative mb-3">
                    ...
                    <input type="text" x-model="rosterSearch" placeholder="Search driver, phone, vehicle…"
                        class="form-input !py-2 pl-9 text-sm">
                </div>
                    ...
                    <template x-for="d in filteredDrivers" :key="d.id">
                        <div @click="focusDriver(d)"
                            ...
                            <div class="relative shrink-0">
                                ...
                                <span
                                    class="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full border-2 border-white dark:border-[#0e1726]"
                                    :class="d.is_online ? (d.on_trip ? 'bg-info' : 'bg-success') :
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <template x-if="d.verified">
                                        <svg class="h-3.5 w-3.5 shrink-0 text-info" fill="currentColor"
                                            viewBox="0 0 20 20">
                                ...
                                <p class="truncate text-xs text-gray-500" x-text="d.vehicle_number || d.phone || '—'">
                                </p>
                            </div>
                            {{-- Status --}}
                            <div class="shrink-0 text-right">
                                <template x-if="d.on_trip">
                                    <span
                                        class="inline-flex items-center gap-1 rounded-full bg-info/10 px-2 py-0.5 text-[10px] font-bold text-info">On
                                        trip</span>
                                </template>
                                <template x-if="d.is_online && !d.on_trip">
                                    <span
                                        class="inline-flex items-center gap-1 rounded-full bg-success/10 px-2 py-0.5 text-[10px] font-bold text-success">Online</span>
                                </template>
                                <template x-if="!d.is_online">
                                    <span
                                        class="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-bold text-gray-500 dark:bg-gray-800">Offline</span>
                                </template>
                                <p class="mt-1 text-[10px] text-gray-400"
                                    x-text="d.is_online ? (d.online_since_human ? d.online_since_human + ' online' : '') : (d.last_seen_human ? d.last_seen_human + ' ago' : '')">
                                </p>
                            </div>
                        </div>
                    </template>
                    <div x-show="filteredDrivers.length === 0" class="py-10 text-center text-sm text-gray-400">No
                        drivers match.</div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
            ...
            <div class="panel lg:col-span-2">
                <div class="mb-4 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">Rides currently in progress — click to
                            inspect on the map</p>
                    </div>
                    <span class="rounded-full bg-info/10 px-2.5 py-0.5 text-xs font-bold text-info"><span
                            x-text="data.active_trips.length"></span> active</span>
                </div>
                                    ...
                                    <td>
                                        <template x-if="t.driver">
                                            <div class="flex items-center gap-2">
                                                <div class="flex h-7 w-7 items-center justify-center rounded-full bg-success/10 text-[10px] font-bold text-success"
                                                    x-text="t.driver.initials"></div>
                                                <span class="whitespace-nowrap text-sm font-medium"
                                                    x-text="t.driver.name"></span>
                                            </div>
                                        </template>
                                        <template x-if="!t.driver"><span
                                                class="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-500 dark:bg-gray-800">Unassigned</span></template>
                                    </td>
                                    <td>
                                        <div class="flex items-center gap-2">
                                            <div class="flex h-7 w-7 items-center justify-center rounded-full bg-info/10 text-[10px] font-bold text-info"
                                                x-text="t.passenger.initials"></div>
                                            <span class="whitespace-nowrap text-sm font-medium"
                                                x-text="t.passenger.name || 'N/A'"></span>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="max-w-[220px]">
                                            <p class="truncate text-xs text-gray-500" :title="t.pickup"
                                                x-text="t.pickup || '—'"></p>
                                            <p class="truncate text-xs font-medium text-gray-700 dark:text-gray-300"
                                                :title="t.drop" x-text="t.drop || '—'"></p>
                                        </div>
                                    </td>
                                    <td class="text-center">
                                        <span
                                            class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold"
                                            :class="statusClass(t.status_bucket)">
                                            <span class="h-1.5 w-1.5 rounded-full"
                                                :class="statusDot(t.status_bucket)"></span>
                                            <span x-text="t.status_label"></span>
                                        </span>
                                    </td>
                                    <td class="text-right"><span class="font-bold text-gray-900 dark:text-white"
                                            x-text="t.fare !== null ? '₹' + formatNum(t.fare) : '—'"></span></td>
                                </tr>
                            </template>
                            <tr x-show="data.active_trips.length === 0">
                                <td colspan="6" class="py-10 text-center text-sm text-gray-400">No active trips
                                    right now.</td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <div class="panel">
                <h2 class="mb-4 text-lg font-bold text-gray-900 dark:text-white"
                    x-text="detail ? (detail.kind === 'driver' ? 'Driver Detail' : 'Trip Detail') : 'Inspector'"></h2>

                {{-- Empty state --}}
                <div x-show="!detail" class="flex flex-col items-center justify-center py-16 text-center">
                    <svg class="mb-3 h-12 w-12 text-gray-200 dark:text-gray-700" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor" stroke-width="1.2">
                        <path d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" />
                    </svg>
                    <p class="text-sm text-gray-400">Select a driver or trip to inspect their live location, route and
                        details here.</p>
                </div>
                ...
                <template x-if="detail && detail.kind === 'driver'">
                    <div class="space-y-4">
                        <div class="flex items-center gap-3">
                            <div class="flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-primary to-secondary text-lg font-bold text-white"
                                x-text="detail.data.initials"></div>
                            <div class="min-w-0">
                                <p class="truncate text-base font-bold text-gray-900 dark:text-white"
                                    x-text="detail.data.name"></p>
                            ...
                            <span class="ltr:ml-auto rtl:mr-auto rounded-full px-2.5 py-1 text-xs font-bold"
                                :class="detail.data.is_online ? 'bg-success/10 text-success' :
                                    'bg-gray-100 text-gray-500 dark:bg-gray-800'"
                                x-text="detail.data.is_online ? 'Online' : 'Offline'"></span>
                        </div>
                        <dl class="grid grid-cols-2 gap-3 text-sm">
                            <div class="rounded-lg bg-gray-50 p-3 dark:bg-white/[0.03]">
                                <dt class="text-[11px] text-gray-400">Online since</dt>
                                <dd class="font-semibold text-gray-800 dark:text-gray-200"
                                    x-text="detail.data.online_since_label || '—'"></dd>
                            </div>
                            <div class="rounded-lg bg-gray-50 p-3 dark:bg-white/[0.03]">
                                <dt class="text-[11px] text-gray-400">Last seen</dt>
                                <dd class="font-semibold text-gray-800 dark:text-gray-200"
                                    x-text="detail.data.last_seen_human ? detail.data.last_seen_human + ' ago' : '—'">
                                </dd>
                            </div>
                            <div class="rounded-lg bg-gray-50 p-3 dark:bg-white/[0.03]">
                                <dt class="text-[11px] text-gray-400">Vehicle</dt>
                                <dd class="font-semibold text-gray-800 dark:text-gray-200"
                                    x-text="detail.data.vehicle_number || '—'"></dd>
                            </div>
                            <div class="rounded-lg bg-gray-50 p-3 dark:bg-white/[0.03]">
                                <dt class="text-[11px] text-gray-400">Rating</dt>
                                <dd class="font-semibold text-gray-800 dark:text-gray-200"><span
                                        x-text="detail.data.rating || '—'"></span> ★</dd>
                            </div>
                            <div class="col-span-2 rounded-lg bg-gray-50 p-3 dark:bg-white/[0.03]">
                                <dt class="text-[11px] text-gray-400">Live location</dt>
                        ...
                        <template x-if="detail.data.current_trip">
                            <div class="rounded-xl border border-info/30 bg-info/5 p-3">
                                <p class="mb-1 text-xs font-bold uppercase text-info">Current trip · #<span
                                        x-text="detail.data.current_trip.id"></span></p>
                                <p class="text-xs text-gray-500" x-text="detail.data.current_trip.pickup"></p>
                                <p class="text-xs font-medium text-gray-700 dark:text-gray-300"
                                    x-text="detail.data.current_trip.drop"></p>
                                <button type="button" class="btn btn-sm btn-outline-info mt-2 w-full"
                                    @click="focusTrip(detail.data.current_trip)">Inspect trip</button>
                            </div>
                        </template>
                    </div>
                </template>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between">
                            ...
                            <span
                                ...
                                <span class="h-1.5 w-1.5 rounded-full"
                                    :class="statusDot(detail.data.status_bucket)"></span>
                        ...
                        <div class="space-y-0">
                            <div class="flex gap-3">
                                ...
                                <div class="pb-3">
                                    <p class="text-[11px] font-bold uppercase text-gray-400">Pickup</p>
                                    <p class="text-sm text-gray-700 dark:text-gray-300"
                                        x-text="detail.data.pickup || '—'"></p>
                                </div>
                            </div>
                            <div class="flex gap-3">
                                <div class="flex flex-col items-center"><span
                                        class="h-3 w-3 rounded-full bg-danger ring-4 ring-danger/15"></span></div>
                                <div>
                                    <p class="text-[11px] font-bold uppercase text-gray-400">Drop-off</p>
                                    <p class="text-sm text-gray-700 dark:text-gray-300"
                                        x-text="detail.data.drop || '—'"></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="rounded-xl bg-gray-50 p-3 dark:bg-white/[0.03]">
                            <p class="mb-2 text-[11px] font-bold uppercase text-gray-400">Passenger dropped off / on
                                board</p>
                            <div class="flex items-center gap-3">
                                <div class="flex h-9 w-9 items-center justify-center rounded-full bg-info/10 text-xs font-bold text-info"
                                    x-text="detail.data.passenger.initials"></div>
                                <div class="min-w-0">
                                    <p class="truncate text-sm font-semibold text-gray-800 dark:text-gray-200"
                                        x-text="detail.data.passenger.name || 'N/A'"></p>
                                    <p class="text-xs text-gray-500"
                                        x-text="detail.data.passenger.phone || 'No phone'"></p>
                                </div>
                            </div>
                            <template x-if="detail.data.passenger.booked_for_someone_else">
                                <p class="mt-2 rounded-md bg-warning/10 px-2 py-1 text-xs text-warning">
                                    Booked for: <span class="font-semibold"
                                        x-text="detail.data.passenger.booked_for_name || 'Someone else'"></span>
                                    <span x-show="detail.data.passenger.booked_for_phone"
                                        x-text="' · ' + detail.data.passenger.booked_for_phone"></span>
                                </p>
                            </template>
                        </div>
                        ...
                        <dl class="grid grid-cols-2 gap-3 text-sm">
                            <div class="rounded-lg bg-gray-50 p-3 dark:bg-white/[0.03]">
                                <dt class="text-[11px] text-gray-400">Fare</dt>
                                <dd class="font-semibold text-gray-800 dark:text-gray-200"
                                    x-text="detail.data.fare !== null ? '₹' + formatNum(detail.data.fare) : '—'"></dd>
                            </div>
                            <div class="rounded-lg bg-gray-50 p-3 dark:bg-white/[0.03]">
                                <dt class="text-[11px] text-gray-400">Distance</dt>
                                <dd class="font-semibold text-gray-800 dark:text-gray-200"
                                    x-text="detail.data.distance || '—'"></dd>
                            </div>
                            <div class="rounded-lg bg-gray-50 p-3 dark:bg-white/[0.03]">
                                <dt class="text-[11px] text-gray-400">Driver</dt>
                                <dd class="truncate font-semibold text-gray-800 dark:text-gray-200"
                                    x-text="detail.data.driver ? detail.data.driver.name : 'Unassigned'"></dd>
                            </div>
                            <div class="rounded-lg bg-gray-50 p-3 dark:bg-white/[0.03]">
                                <dt class="text-[11px] text-gray-400">Placed</dt>
                                <dd class="font-semibold text-gray-800 dark:text-gray-200"
                                    x-text="detail.data.created_label || '—'"></dd>
                            </div>
                            <div class="col-span-2 rounded-lg bg-gray-50 p-3 dark:bg-white/[0.03]">
                                <dt class="text-[11px] text-gray-400">Last known live location</dt>
                                <dd class="font-mono text-xs font-semibold text-gray-800 dark:text-gray-200"
                                    x-text="detail.data.has_live_location ? (detail.data.live_lat.toFixed(5) + ', ' + detail.data.live_lng.toFixed(5)) : 'Not available'">
                                </dd>
                            </div>
                        </dl>
                    </div>
        ...
        <div class="panel">
            <div class="mb-4 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">Latest rides with pickup, drop-off &amp;
                        passenger — for quick troubleshooting</p>
                </div>
                ...
                <table class="table-hover">
                    ...
                    <tbody>
                        <template x-for="t in data.recent_trips" :key="t.id">
                            <tr>
                                <td><span class="font-bold text-primary" x-text="'#' + t.id"></span></td>
                                <td><span class="whitespace-nowrap text-sm"
                                        x-text="t.driver ? t.driver.name : '—'"></span></td>
                                <td><span class="whitespace-nowrap text-sm" x-text="t.passenger.name || 'N/A'"></span>
                                </td>
                                <td>
                                ...
                                <td class="text-right"><span class="font-bold text-gray-900 dark:text-white"
                                        x-text="t.fare !== null ? '₹' + formatNum(t.fare) : '—'"></span></td>
                                <td class="whitespace-nowrap text-xs text-gray-500" x-text="t.created_human || '—'">
                                </td>
                                <td class="text-center"><button type="button"
                                        class="text-xs font-semibold text-primary hover:underline"
                                        @click="focusTrip(t)">Inspect</button></td>
                            </tr>
                        </template>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('godsEye', (cfg) => ({
                ...
                get kpiCards() {
                    ...
                    const ic = (path) =>
                        `<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">${path}</svg>`;
                    return [{
                            key: 'total',
                        ...
                        {
                            key: 'online',
                            value: k.online_drivers,
                            label: 'Online Now',
                            iconWrap: 'bg-success/10 text-success',
                            icon: ic(
                                '<path d="M12 21a9 9 0 100-18 9 9 0 000 18z"/><path d="M9 12l2 2 4-4"/>'
                            )
                        },
                        {
                            key: 'ontrip',
                            value: k.drivers_on_trip,
                            label: 'On Trip',
                            iconWrap: 'bg-info/10 text-info',
                            icon: ic(
                                '<path d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H18.75M2.25 14.25h1.5m13.5 0h1.5m-16.5 0V6.375c0-.621.504-1.125 1.125-1.125h3.026a3 3 0 012.25 1.016l.272.341a3 3 0 002.25 1.016h5.577c.621 0 1.125.504 1.125 1.125V14.25"/>'
                            )
                        },
                        {
                            key: 'offline',
                            value: k.offline_drivers,
                            label: 'Offline',
                            iconWrap: 'bg-gray-200 text-gray-500 dark:bg-gray-700',
                            icon: ic(
                        ...
                        {
                            key: 'today',
                            value: k.online_today,
                            label: 'Came Online Today',
                ...
                get filteredDrivers() {
                    ...
                    return this.data.drivers.filter(d => {
                        ...
                        if (!q) return true;
                        return [d.name, d.phone, d.vehicle_number, d.city].some(v => (v ||
                ...
                /* ── Google Maps ── */
                loadGoogleMaps() {
                    // Surface auth failures (bad key / Maps JS API not enabled /
                    // referrer blocked) instead of leaving the map silently blank.
                    window.gm_authFailure = () => { this.mapStatus = 'error'; };
                    ...
                    s.async = true;
                    s.defer = true;
                    s.src = 'https://maps.googleapis.com/maps/api/js?key={{ $mapKey }}&callback=__godsEyeMapInit&loading=async';
                    s.onerror = () => { this.mapStatus = 'error'; };
