...
<div class="space-y-5">
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            <form method="GET" action="{{ route('admin.coupons.index') }}" class="relative">
                ...
                <tbody>
                    @forelse($coupons as $coupon)
                        <tr>
                            ...
                            <td class="font-semibold">{{ $coupon->title }}</td>
                            <td><code class="px-2 py-1 text-xs font-bold rounded bg-gray-100 dark:bg-gray-700">{{ $coupon->code }}</code></td>
                            <td>{{ $coupon->amount }}{{ $coupon->type === 'percentage' ? '%' : '' }}</td>
                            <td>
                                <span class="badge {{ $coupon->type === 'percentage' ? 'bg-info' : 'bg-warning' }}">
                                    {{ ucfirst($coupon->type === 'fix' ? 'Fixed' : $coupon->type) }}
                                </span>
                            </td>
                            <td class="whitespace-nowrap">{{ $coupon->validity ? \Carbon\Carbon::parse($coupon->validity)->format('d M Y') : '-' }}</td>
                            <td>
                                <form method="POST" action="{{ route('admin.coupons.toggle-status', $coupon) }}" class="inline">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="badge {{ $coupon->enable ? 'bg-success' : 'bg-danger' }}">
                                        {{ $coupon->enable ? 'Active' : 'Inactive' }}
                                    </button>
                                </form>
                            </td>
                            <td class="text-center">
                                <div class="flex items-center justify-center gap-3">
                                    <a href="{{ route('admin.coupons.edit', $coupon) }}" class="hover:text-primary">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-4 text-gray-500">No coupons found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
