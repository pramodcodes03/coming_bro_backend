...
<div class="space-y-5">
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            <form method="GET" action="{{ route('admin.faqs.index') }}" class="relative">
                ...
                <tbody>
                    @forelse($faqs as $faq)
                        <tr>
                            ...
                            <td class="font-semibold">{{ $faq->title }}</td>
                            <td>
                                <form method="POST" action="{{ route('admin.faqs.toggle-status', $faq) }}" class="inline">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="badge {{ $faq->enable ? 'bg-success' : 'bg-danger' }}">
                                        {{ $faq->enable ? 'Active' : 'Inactive' }}
                                    </button>
                                </form>
                            </td>
                            <td class="text-center">
                                <div class="flex items-center justify-center gap-3">
                                    <a href="{{ route('admin.faqs.edit', $faq) }}" class="hover:text-primary">
