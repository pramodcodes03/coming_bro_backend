    ...
    <div class="space-y-6" x-data="{
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-3">
                <button type="button" @click="planOpen = true"
        ...
        <div class="panel">
            <div class="flex flex-col gap-3 mb-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex items-center self-start gap-2">
                    <button type="button" @click="planOpen = true"
                        class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white rounded-lg bg-gradient-to-r from-[#018DBD] to-[#13C3C3] shadow-sm hover:opacity-90 transition">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
                        Assign Plan
                    </button>
                    <button type="button" @click="open = true"
                        class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white rounded-lg bg-gradient-to-r from-emerald-500 to-green-500 shadow-sm hover:opacity-90 transition">
    ...
    <div x-show="open" x-cloak
        ...
        <div x-show="planOpen" x-cloak
            ...
            <div class="relative w-full max-w-md overflow-hidden bg-white shadow-2xl dark:bg-gray-900 rounded-2xl"
                ...
                <!-- Gradient header -->
                <div class="relative px-6 py-5 text-white bg-gradient-to-r from-[#018DBD] to-[#13C3C3]">
                ...
                @else
                <form method="POST" action="{{ route('admin.drivers.assign-plan', $driver->id) }}" class="px-6 py-5 space-y-5">
                    @csrf

                    <!-- Plan select -->
                    <div>
                        <label for="assign_plan_id" class="block mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">Choose a plan</label>
                        <select id="assign_plan_id" name="recharge_plan_id" x-model="selectedPlanId" required
                            class="w-full px-4 py-3 text-sm text-gray-900 bg-gray-50 border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700 dark:text-white focus:ring-2 focus:ring-[#018DBD] focus:border-[#018DBD] outline-none">
                            <option value="">— Select a plan —</option>
                            @foreach($rechargePlans as $p)
                                <option value="{{ $p->id }}">
                                    {{ $p->label }} — {{ (int) $p->rides }} rides · ₹{{ number_format((float) $p->price, 2) }}{{ $p->validity_days ? ' · '.$p->validity_days.'d' : ' · no expiry' }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <!-- Note -->
                    <div>
                        <label for="assign_plan_note" class="block mb-1.5 text-sm font-semibold text-gray-700 dark:text-gray-300">Reason / note <span class="font-normal text-gray-400">(optional)</span></label>
                        <input id="assign_plan_note" type="text" name="note" maxlength="255"
                    ...
                    <div class="flex items-center gap-3 p-3.5 rounded-lg bg-[#018DBD]/10 border border-[#018DBD]/20" x-show="selectedPlan">
                        ...
                        <p class="text-sm text-[#01698c] dark:text-[#13C3C3]">
                            Driver will have
                            <span class="font-bold" x-text="{{ (int) ($driver->remaining_rides ?? 0) }} + (selectedPlan ? selectedPlan.rides : 0)"></span>
                            rides remaining (currently {{ (int) ($driver->remaining_rides ?? 0) }}).
                        </p>
                    </div>
